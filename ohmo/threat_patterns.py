...
_PATTERNS: List[Tuple[str, str, str]] = [
    ...
    (
        r"\.codex/auth\.json|\.worfalomey_oauth\.json|\.ohmo/gateway\.json",
        "ohmo_secret_files",
        "strict",
    ),
    # Tampering with agent instruction / persona / memory files.
    (
        r"(update|modify|edit|write|change|append|add\s+to)\s+.*(?:AGENTS\.md|CLAUDE\.md|\.cursorrules|\.clinerules|soul\.md|user\.md|MEMORY\.md)",
        "agent_config_mod",
        "strict",
    ),
    (
        r"(update|modify|edit|write|change|append|add\s+to)\s+.*\.ohmo/(gateway\.json|soul\.md|user\.md|BOOTSTRAP\.md)",
        "ohmo_config_mod",
        "strict",
    ),
    # ── Hardcoded secrets ────────────────────────────────────────────
