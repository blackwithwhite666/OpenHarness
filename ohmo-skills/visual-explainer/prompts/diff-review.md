...
- Commit hash: that specific commit's diff (`arc show <hash>`)
- `HEAD`: uncommitted changes only (`arc diff` and `arc diff --staged`)
...
- `arc diff --stat <ref>` for file-level overview
- `arc diff --name-status <ref> --` for new/modified/deleted files (separate src from tests)
- Line counts: compare key files between `<ref>` and working tree (`arc show <ref>:file | wc -l` vs `wc -l`)
