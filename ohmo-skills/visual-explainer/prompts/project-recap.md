...
- Shorthand like `2w`, `30d`, `3m`: parse to arc's `--since` format (`2w` → `"2 weeks ago"`, `30d` → `"30 days ago"`, `3m` → `"3 months ago"`)
...
2. **Recent activity.** `arc log --oneline --since=<window>` for commit history. `arc log --stat --since=<window>` for file-level change scope. `arc shortlog -sn --since=<window>` for contributor activity. Identify which areas of the codebase were most active.

3. **Current state.** Check for uncommitted changes (`arc status`). Check for stale branches (`arc branch --no-merged`). Look for TODO/FIXME comments in recently changed files. Read progress docs if they exist (`~/.agent/memory/{project}/progress.md`, `~/.pi/agent/memory/{project}/progress.md`, `.pi/todos/`, or similar).
...
- For each, cite the source: the arc command output that produced it, or the file:line where you read it
...
3. **Recent activity** — not raw arc log. A human-readable narrative grouped by theme: feature work, bug fixes, refactors, infrastructure. Timeline visualization with the most significant changes called out. For each theme, a one-sentence summary of what happened and why it mattered.
