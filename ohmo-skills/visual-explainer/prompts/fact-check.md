...
- **Temporal**: arc history claims, commit attributions, timeline entries
...
- For claims about arc history: re-run arc commands (`arc diff --stat`, `arc log`, `arc diff --name-status`, etc.) and compare output against the document's numbers
- For diff-reviews: read both the ref version (`arc show <ref>:file`) and working tree version to verify before/after claims aren't swapped or fabricated
- For plan docs: verify that files, functions, and types the plan references actually exist and behave as described
- For project-recaps: re-run `arc log` commands to verify activity narrative and timeline
