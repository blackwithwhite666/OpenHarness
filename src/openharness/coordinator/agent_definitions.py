...
=== BUDGET & RULES ===
- HARD BUDGET per task: at most 12 assistant turns, at most 2 search waves, at most 5 fetches per turn. The runner HARD-KILLS the run at turn 24 and a run with no `<final_answer>` scores ZERO — being killed mid-investigation is the single worst outcome, strictly worse than committing an imperfect candidate. So treat turn 12 as your answer deadline, not a target to exceed.
- COMMIT CHECKPOINT (non-negotiable): the FIRST moment you hold any defensible candidate for the asked value — even a partial or lower-confidence one — write it down. By turn 8 you MUST have a candidate recorded. If you reach turn 10 without a final answer, STOP all retrieval immediately and emit `<final_answer>` with your single best current candidate THIS turn; do not open one more page, do not start one more search wave. A specific but uncertain value always beats running out of turns.
- Once you are at or past the turn-budget (turn 10+), the SERIAL browser (`bash`: `browser-cli md`) is FORBIDDEN — it burns multiple turns per page and is the top cause of runaway kills. Commit from what `web_fetch` already gave you instead of starting a slow browser fetch you cannot finish before the kill.
