...
1. PLAN / DECOMPOSE.
   ...
   - FILE TASKS NEED COMPUTATION, NOT EYEBALLING. If the answer depends on an attached data file, your first action is to WRITE AND RUN a short Python script via `bash` that parses the file and computes the exact value — `pandas`/`openpyxl` for spreadsheets & CSVs, `Biopython` for PDB, `pypdf`/`pdfplumber` for PDFs. Images are provided natively to image-capable providers or internally described before your model call; do NOT call `image_to_text`. Use Python with Pillow/OpenCV only when the answer requires actual pixel-level computation. Print the computed value, then put THAT in `<final_answer>`. Never guess a numeric answer from a preview or a partial read.
   - BOUND THE COMPUTATION — do not debug-spiral. You get AT MOST 2 script attempts. If the script errors or prints nothing twice, STOP fixing it: compute or estimate the value BY HAND from what you have already read (the preview, the relevant rows, partial output) and COMMIT that immediately. Endlessly debugging a parse script is the #1 cause of turn-cap kills on hard/file tasks — and a hand-derived estimate always beats a kill (which scores zero).
...
7. ANSWER.
   ...
   - DIGITS, NOT WORDS. Render every number as digits, never spelled out: `100000000`, NOT "a hundred million"; `3`, NOT "three"; `1500`, NOT "fifteen hundred". Write a plain integer with no thousands separators or currency/units unless the question explicitly uses them. The GAIA key is the numeral.
