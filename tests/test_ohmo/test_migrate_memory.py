...
def test_import_entry_supports_active_archived_and_fts(tmp_path: Path) -> None:
    ...
    active = catalog.import_entry("active_note", "Active note", "well over budget")
    ...
    assert [record.slug for record in catalog.search("migration token", 5)] == ["old_note"]
...
def test_import_entry_is_idempotent_and_never_clobbers(tmp_path: Path) -> None:
    ...
    assert catalog.import_entry("profile", "Profile", "original").ok
    ...
    conflict = catalog.import_entry("profile", "Profile", "replacement")
...
def test_import_entry_validates_source_and_archive_status(tmp_path: Path) -> None:
    ...
    assert catalog.import_entry("bad_source", "Bad", "value", source="external").ok is False
    assert (
    ...
    assert catalog.list(include_archived=True) == []
...
def test_apply_imports_all_entries_without_touching_markdown(
    ...
    catalog = MemoryCatalog(db_path=legacy_workspace.db_path)
    records = {record.slug: record for record in catalog.list(include_archived=True)}
...
def test_second_apply_skips_every_duplicate(legacy_workspace: LegacyWorkspace) -> None:
    ...
    catalog = MemoryCatalog(db_path=legacy_workspace.db_path)
    assert len(catalog.list(include_archived=True)) == 3
...
def test_conflict_is_reported_without_clobbering(
    ...
    catalog = MemoryCatalog(db_path=legacy_workspace.db_path)
    assert catalog.import_entry("timezone", "Existing timezone", "different content").ok
